...
function makeObj(...args: any[]): {[name: string]: any} {
...
describe('Assert', function () {
  ...
  it('should do deep equal', function () {
    const now = 1544200539595;
    ...
    notEqual(new Uint8Array([1, 2, 3, 4]).buffer, new Uint8Array([1, 2, 3]).buffer);
    notEqual(new Uint8Array([1, 2]).buffer, new Uint8Array([1, 2, 3]).buffer);
    notEqual(new Uint8Array([2, 2, 3]).buffer, new Uint8Array([1, 2, 3]).buffer);
  ...
  it('should test buffers', () => {
    assert.bufferEqual(Buffer.from('010203', 'hex'), Buffer.from('010203', 'hex'));
    assert.notBufferEqual(Buffer.from('010203', 'hex'), Buffer.from('01020304', 'hex'));
    assert.notBufferEqual(Buffer.from('010203', 'hex'), Buffer.from('0102', 'hex'));
    assert.notBufferEqual(Buffer.from('010203', 'hex'), Buffer.from('020203', 'hex'));

    assert.throws(() => {
      assert.notBufferEqual(Buffer.from('010203', 'hex'), Buffer.from('010203', 'hex'));
    });

    assert.throws(() => {
      assert.bufferEqual(Buffer.from('010203', 'hex'), Buffer.from('01020304', 'hex'));
    });

    assert.throws(() => {
      assert.bufferEqual(Buffer.from('010203', 'hex'), Buffer.from('0102', 'hex'));
    });

    assert.throws(() => {
      assert.bufferEqual(Buffer.from('010203', 'hex'), Buffer.from('020203', 'hex'));
    });
