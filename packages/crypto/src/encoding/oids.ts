/*!
 * oids - OIDs encoding for javascript
...
export namespace oids {
  ...
  export const attrs: Record<string, string> = {
    ...
    POSTALCODE: '2.5.4.17',
    EMAIL: '1.2.840.113549.1.9',
  };

  export const attrsByVal: Record<string, string> = {
  ...
  export const keyAlgs: Record<string, string> = {
  ...
  export const keyAlgsByVal: Record<string, string> = {
    [keyAlgs.DH]: 'DH',
    [keyAlgs.DSA]: 'DSA',
    [keyAlgs.DSA_ALT]: 'DSA',
    [keyAlgs.RSA]: 'RSA',
  ...
  export const hashesByVal: Record<string, string> = {
  ...
  export const curves: Record<string, string> = {
  ...
  export const curvesByVal: Record<string, string> = {
    [curves.P192]: 'P192',
    [curves.P224]: 'P224',
    [curves.P256]: 'P256',
    [curves.P384]: 'P384',
  ...
  export const sigAlgs: Record<string, string> = {
  ...
  export const sigAlgsByVal: Record<string, string> = {
  ...
  export const sigToHash: Record<string, string | null> = {
    ...
    [sigAlgs.ECDSASHA256]: hashes.SHA256,
  ...
  export const exts: Record<string, string> = {
    NS_CERT_TYPE: '2.16.840.1.113730.1.1',
    KEY_ATTRIBUTES: '2.5.29.2', // obsolete, use .37 or .15
    KEY_USAGE_RESTRICTION: '2.5.29.4', // obsolete, use .37 or .15
    SUBTREES_CONSTRAINT: '2.5.29.6', // obsolete, use .30
  ...
  const extsByVal: Record<string, string> = {
    [exts.NS_CERT_TYPE]: 'NS_CERT_TYPE',
    [exts.KEY_ATTRIBUTES]: 'KEY_ATTRIBUTES', // obsolete, use .37 or .15
    [exts.KEY_USAGE_RESTRICTION]: 'KEY_USAGE_RESTRICTION', // obsolete, use .37 or .15
    [exts.SUBTREES_CONSTRAINT]: 'SUBTREES_CONSTRAINT', // obsolete, use .30
    [exts.SUBJECT_DIRECTORY_ATTRIBUTES]: 'SUBJECT_DIRECTORY_ATTRIBUTES',
    [exts.SUBJECT_KEY_IDENTIFIER]: 'SUBJECT_KEY_IDENTIFIER',
    [exts.KEY_USAGE]: 'KEY_USAGE',
    [exts.PRIVATE_KEY_USAGE_PERIOD]: 'PRIVATE_KEY_USAGE_PERIOD',
    [exts.SUBJECT_ALT_NAME]: 'SUBJECT_ALT_NAME',
    [exts.ISSUER_ALT_NAME]: 'ISSUER_ALT_NAME',
    [exts.BASIC_CONSTRAINTS]: 'BASIC_CONSTRAINTS',
    [exts.CRL_NUMBER]: 'CRL_NUMBER',
    [exts.CRL_REASON]: 'CRL_REASON',
    [exts.EXPIRATION_DATE]: 'EXPIRATION_DATE',
    [exts.INSTRUCTION_CODE]: 'INSTRUCTION_CODE',
    [exts.INVALIDITY_DATE]: 'INVALIDITY_DATE',
    [exts.DELTA_CRL_INDICATOR]: 'DELTA_CRL_INDICATOR',
    [exts.ISSUING_DISTRIBUTION_POINT]: 'ISSUING_DISTRIBUTION_POINT',
    [exts.CERTIFICATE_ISSUER]: 'CERTIFICATE_ISSUER',
    [exts.NAME_CONSTRAINTS]: 'NAME_CONSTRAINTS',
    [exts.CRL_DISTRIBUTION_POINTS]: 'CRL_DISTRIBUTION_POINTS',
    [exts.CERTIFICATE_POLICIES]: 'CERTIFICATE_POLICIES',
    [exts.POLICY_MAPPINGS]: 'POLICY_MAPPINGS',
    [exts.AUTHORITY_KEY_IDENTIFIER]: 'AUTHORITY_KEY_IDENTIFIER',
    [exts.POLICY_CONSTRAINTS]: 'POLICY_CONSTRAINTS',
    [exts.EXT_KEY_USAGE]: 'EXT_KEY_USAGE',
    [exts.FRESHEST_CRL]: 'FRESHEST_CRL',
    [exts.INHIBIT_ANY_POLICY]: 'INHIBIT_ANY_POLICY',
  };

  Object.assign(attrs, exts);
  Object.assign(attrsByVal, extsByVal);

  const regoid = /^[0-9.]+$/;

  export function findIdByName(name: string) {
    return attrs[name] ?? keyAlgs[name] ?? hashes[name] ?? curves[name] ?? sigAlgs[name] ?? exts[name];
  }

  export function findNameById(id: string) {
    return (
      attrsByVal[id] ?? keyAlgsByVal[id] ?? hashesByVal[id] ?? curvesByVal[id] ?? sigAlgsByVal[id] ?? extsByVal[id]
    );
  }

  export function findHashBySig(sigIdOrName: string) {
    return sigToHash[foid(sigIdOrName)];
  }
   ...
   * @param oidOrName OID name or OID
   */
  export function foid(oidOrName: string): string {
    const found = findIdByName(oidOrName) || findIdByName(oidOrName.toUpperCase());

    if (regoid.test(oidOrName) && found !== '') {
      return oidOrName;
    }
   ...
   * @param nameOrOid OID name or OID
   */
  export function fname(nameOrOid: string): string {
    const found = findNameById(nameOrOid) || findNameById(nameOrOid.toUpperCase());

    if (!regoid.test(nameOrOid) && found !== '') {
      return nameOrOid;
    }

    return found == null ? nameOrOid : found;
  }

  export function goid(oidOrName: string): string {
    const oid = foid(oidOrName);
    if (!oid) throw new Error('Unknown oid or name: ' + oidOrName);
    return oid;
  }

  export function gname(nameOrOid: string): string {
    const name = fname(nameOrOid);
    if (!name) throw new Error('Unknown name or oid: ' + nameOrOid);
    return name;
  }
}
