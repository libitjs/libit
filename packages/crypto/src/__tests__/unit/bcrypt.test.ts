...
describe('Bcrypt', function () {
  describe('Hash192', () => {
    for (const [sPass, sSalt, rounds, sExpect] of hash192) {
      ...
      const expect = Buffer.from(sExpect, 'hex');
      const text = sExpect.slice(0, 32) + '...';

      // eslint-disable-next-line mocha/handle-done-callback
      it(`should derive key (hash192): ${text}`, function (this: Mocha.Context) {
        if (rounds > 8 && !process.env.CI) this.skip();
  ...
  describe('Generate', () => {
    for (const [pass, rounds, salt, expect] of bsd) {
      ...
      // eslint-disable-next-line mocha/handle-done-callback
      it(`should derive hash (bsd): ${text}`, function (this: Mocha.Context) {
        if (rounds > 8 && !process.env.CI) this.skip();
  ...
  describe('Verify', () => {
    for (const [pass, rounds, , expect] of bsd) {
      ...
      // eslint-disable-next-line mocha/handle-done-callback
      it(`should verify hash (bsd): ${text}`, function (this: Mocha.Context) {
        if (rounds > 8 && !process.env.CI) this.skip();
