...
describe('PEM', function () {
  ...
  it('should deserialize PEM from vector', () => {
    ...
    assert.bufferEqual(block.data, Buffer.from('soWLOTC9I8bbnYcwAaLa1sMwEvWrzt1iXkV8F/r8v1M=', 'base64'));
  ...
  it('should decrypt PEM from vector', () => {
    ...
    assert.strictEqual(block.type, 'BCRYPTO MESSAGE');
    assert.strictEqual(block.headers.get('Bcrypto-Header'), 'foo');
    assert.bufferEqual(block.data, Buffer.from('YXxIZ7jipxJVI2/nZsA59F79pkk0s+9cPcAF/lLhsvY=', 'base64'));
  });
  ...
  it('should parse strictly', () => {
    ...
    assert.bufferEqual(data, Buffer.from('soWLOTC9I8bbnYcwAaLa1sMwEvWrzt1iXkV8F/r8v1M=', 'base64'));
